...
contains
  ...
  subroutine getsid(jday, midnight)
    ! Calculate sidereal time at Greenwhich
  ...
  subroutine riset(jday, lon, lat, deltat, yesterday, today, tomorrow, id, time)
    ...
    real(8) :: approx ! apprximate time related to sunset
    ...
    testval = (sin(d2r * h0) - (sin(d2r * lat) * sin(tomorrow(2))) / (cos(d2r * lat) * cos(tomorrow(2))))
    if (abs(testval) > 1.0) then
       ...
       time(1) = transit - ((bigh * r2d) / 360.0) ! sunrise
       time(2) = transit + ((bigh * r2d) / 360.0) ! sunset
