...
# Convert between Julian Days and dates in the modern traditional Borana calendar as described by Marco Bassi
# The main source here is On The Borana Calendrical System: A Preliminary Field Report. Marco Bassi, Current Anthropology 29:4 (1988), pp 619—624
...
from stars import SHERATAN, PROCYON, ALDEBARAN
...
solar_epoch = 1947986.852074074 + (sid_year / 2) + (917 * sid_year) # local Julian Day in the year 1538 AD when the sun was in opposite Sheratan (Lami)
...
def zeq(jday, opp):
    ...
    return get_solar_zpos(jday, SHERATAN, opp, 0)
...
def autumn_crescent(year):
    ...
    # crescent is now the provisional start of the month
    # but we need to clarify it with respect to Baqqalch Sors (Aldebaran)
    # The moon is officially in conjunction with Aldebaran on the third day of the month
    # which means the month begins two days prior
    # See Bassi, 1988
    crescent = max(dayof(crescent), (dayof(llsc(crescent, ALDEBARAN, tz)) - 2))
