...
# Convert between the tradition Indian solar calendar and Julian Day, dating according to the Vikram Samvat, using methods described in the Sūrya Siddhānta
...
from surya_siddhanta import vs as epoch, sunrise, sunset, sankranti, t_sid_year as sid_year, t_rasi as rasi
...
def fromjd(jday):
    '''Convert a Julian Day into a date in the traditional Indian solar calendar (Vikram Samvat)'''
...
def tojd(day, month, year):
    '''Convert a date in the traditional Indian solar calendar (Vikram Samvat) into a Julian Day'''
    day = int(day)
    month = str(month)
    year = int(year)
