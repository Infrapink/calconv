...
def fromjd(jday):
    ...
    # convert years into olympiads
    # add 1 to each because humans don't count from 0
    olympiad = (year // 4) + 1
    year = (year % 4) + 1
    ...
    return (day, month, year, olympiad)

def tojd(day, month, year, olympiad):
    '''Convert an Attic date to a Julian Day'''
    day = int(day) - 1 # subtract 1 because humans don't count from 0
    month = str(month)
    year = int(year) - 1
    olympiad = int(olympiad) - 1

    # account for the olympiad and year
    year += (4 * olympiad)
    jday = nyd(year)
