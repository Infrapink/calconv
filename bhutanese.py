...
# A programme to convert between Julian Days and the traditional Bhutanese calendar
...
def dayof(mcount, tithi):
    ...
    t = tibetan.tibetan.bhutanese(mcount, tithi)
    ans = t[0] + sign(sum(t) - t[0])
    return ans

# astronomical constants
epoch = 2361807 # nominal epoch corresponding to the new moon that fell on 22 April 1754 AD
syn_month = Fraction(167025, 5656)
sid_year = syn_month * 12 * Fraction(67,65)

def tojd(tithi, month, year):
    '''Convert a date in the Bhutanese calendar to a Julian Day.'''
    ...
    mcount = (((year * 12) - 2) * Fraction(67,65)) + Fraction(2,65) # months since the epoch
    if (mcount % 1 > Fraction(60,65)):
    ...
    while (MONTHS[m] != month):
        if (mcount % 1 in (Fraction(57,65), Fraction(58,65))):
            m += 0
        else:
            m += 1
        mcount += Fraction(67,65)
    if ( (leap == True) and (mcount % 1 in (Fraction(57,68), Fraction(58,65)))):
        mcount += 1
...
def fromjd(jday):
    '''Given a Julian Day, compute a date in the Bhutanese calendar'''
    jday = Fraction(jday)

    # compute the year
    year = round((jday - epoch) / sid_year)
    mcount = (((year * 12) - 2) * Fraction(67,65)) + Fraction(2,65)
    if (mcount % 1 > Fraction(60,65)):
        mcount += Fraction(67,65)
    ...
    while( dayof( (mcount + Fraction(67,65)), 0) <= jday):
        if (mcount % 1 in ( Fraction(57,65), Fraction(58,65) )):
            ...
            m += 1
        mcount += Fraction(67,65)
    if (mcount % 1 in (Fraction(59,65), Fraction(60,65))):
        month = "Leap " + MONTHS[m]
    else:
        month = MONTHS[m]
