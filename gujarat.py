...
# A programme to convert between Julian Days and dates in the Gujarati regional calendar
...
epoch = sankranti((vs + (6 * rasi)), 180) # Tulā Saṁkrānti of 0 VS
...
def fromjd(jday):
    ...
    angle = ((cigra * 30) + 180) % 360  # zodiacal angle of the cusp of the star sign we're in; note that in this particular calendar, we start at 180°
...
def tojd(tithi, month, year):
    ...
    m = (MONTHNO[month] - 7) % 12 # number of the star sign that the month begins in

    jday = epoch + (year * sid_year) + (m * rasi) # approximate saṁkrānti of the month specified
    angle = ((m * 30) + 180) % 360 # zodiacal angle of the beginning of the star sign we're interested in
    crescent = newmoon(jday, tz)
