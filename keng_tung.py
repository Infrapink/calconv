...
# A programme to convert between the Thai lunisolar calendar and Julian Days, assuming Keng Tung reckoning of month
...
from months import THAI_LUNAR_NORMAL, KENG_TUNG_LEAP as LEAP
from sea import lny, sid_year, chulasakarat as epoch

NORMAL = THAI_LUNAR_NORMAL[1:] + THAI_LUNAR_NORMAL[:1]
...
def tojd(day, month, year):
    '''Convert a date in the Keng Tung lunisolar calendar to Julian Day'''
    ...
    if (moonums[0] == 384):
        ...
        MONTHS = NORMAL
    monthdays = MONTHDAYS[moonuyms[0]]
    jday = moonums[1]
...
def fromjd(jday):
    '''Convert a Julian Day into a date in the Keng Tung lunisolar calendar'''
    jday = int(jday)
